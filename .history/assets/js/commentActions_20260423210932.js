...
function likeComment(commentId,button,videoId){
     $.post("ajax/likeComment.php", { commentId:commentId , videoId: videoId })
        .done(function(numToChange) {
            ...
            // ✅ NOW inside .done() where dislikeButton is in scope
...
function dislikeComment(commentId,button,videoId){
       $.post("ajax/dislikeComment.php", { commentId: commentId, videoId: videoId })
        .done(function(numToChange) {
            var dislikeButton = $(button);                            // ✅ wrap button first
            var likeButton = dislikeButton.siblings(".likeButton");   // ✅ then get sibling
...
function updateLikesValue(element, num) {
    var likesCountVal = parseInt(element.text()) || 0;
    element.text(likesCountVal + parseInt(num));  // 0 + 1 = 1 ✅
}

function getReplies(commentId,button,videoId){
    $.post(ajax/getCommentReplies.php) , {commentId:commentId , videoId}
