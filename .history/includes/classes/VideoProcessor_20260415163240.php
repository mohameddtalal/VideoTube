...
class VideoProcessor {
    ...
    public function generateTumbnails($filePath) {
        ...
        for($num=1;$num<=$numTuhmbnails;$num++) {
            ...
            $timeToCapture = $interval * $num; //calculate time to capture thumbnail
            $fullThumbnailPath = $pathToThumbnail . $imageName; //full path to thumbnail
            $cmd = "$this->ffmpegPath -i $filePath -ss $timeToCapture -s $thumbnailSize -vframes 1 $fullThumbnailPath 2>&1"; //command to generate thumbnail
            $outputLog = array();
            exec($cmd, $outputLog, $returnCode); //execute command
            if($returnCode != 0) { //check if there was an error
                foreach($outputLog as $line) {
