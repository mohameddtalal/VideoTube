...
class VideoProcessor {
    ...
    public function convertVideoToMp4($tempFilePath,$finalFilePath) {
        $ffmpegPath = "C:\\xampp\\htdocs\\VideoTube\\ffmpeg\\windows\\ffmpeg.exe"; //path to ffmpeg
        $cmd = "$ffmpegPath -i $tempFilePath -c:v libx264 -preset slow -crf 22 -c:a aac -b:a 128k $finalFilePath"; //command to convert video to mp4
        exec($cmd, $output, $returnVar); //execute command
        if($returnVar != 0) { //check if there was an error
            echo "Error converting video to mp4. Return code: " . $returnVar;
            return false;
        }
        return true;
    }
