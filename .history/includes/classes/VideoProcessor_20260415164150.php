...
class VideoProcessor {
    ...
    private $ffprobePath = "C:/xampp/htdocs/VideoTube/ffmpeg/windows/ffprobe.exe"; //path to ffprobe
    public function __construct($con) {
        $this->con = $con;
        $this->ffmpegPath = realpath($this->ffmpegPath); //get real path to ffmpeg
        $this->ffprobePath = realpath($this->ffprobePath); //get real path to ffprobe
    }
    ...
    public function upload($videoUploadData) {
        ...
        if(move_uploaded_file($videoData["tmp_name"],$tempFilePath)) { //move file to target directory
          ...
          if(!$this->insertVideoData($videoUploadData,$finalFilePath)) { //insert video data into database
            ...
            if(!$this->generateTumbnails($finalFilePath)) { //generate thumbnails
                echo "Error generating thumbnails.";
                return false;
            }
        }
    ...
    public function generateTumbnails($filePath) {
        ...
        $numTuhmbnails=3;
        $pathToThumbnail = "uploads/videos/thumbnails/"; //store thumbnails here
        $duration= $this->getVideoDuration($filePath); //get video duration
        $videoId = $this->con->lastInsertId(); //get last inserted video id
        $this->updateDuration($duration,$videoId); //update video duration in database

        for($num=1;$num<=$numTuhmbnails;$num++) {
            $imageName = uniqid() . ".jpg"; //unique image name
            $interval = ($duration * 0.8) / $numTuhmbnails * $num; //calculate interval between thumbnails
            $fullThumbnailPath = "$pathToThumbnail/$videoId-$imageName" ; //full path to thumbnail

                $cmd = "$this->ffmpegPath -i $filePath -ss $interval -s $thumbnailSize -vframes 1 $fullThumbnailPath 2>&1"; //path to ffmpeg
                $outputLog = array();
                exec($cmd, $outputLog, $returnCode); //execute command
                if($returnCode != 0) { //check if there was an error
                    foreach($outputLog as $line) {
                ...
                $query = $this->con->prepare("INSERT INTO thumbnails(videoId,filePath,isSelected) VALUES(:videoId,:filePath,:isSelected)"); //insert thumbnail data into database
                $isSelected = ($num == 1) ? 1 : 0; //set first thumbnail as selected
                $query->bindParam(":videoId",$videoId); 
                $query->bindParam(":filePath",$fullThumbnailPath);
                $query->bindParam(":isSelected",$isSelected);
                $query->execute();
                
            if(!$this->insertThumbnailData($videoId,$fullThumbnailPath)) { //insert thumbnail data into database
                echo "Error inserting thumbnail data into database.";
                return false;
            }
        }
    ...
    return shell_exec("$this->ffprobePath -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 \"$filePath\""); //get video duration using ffmpeg
