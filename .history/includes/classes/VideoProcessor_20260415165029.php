...
class VideoProcessor {
    ...
    public function generateTumbnails($filePath) {
        ...
        for($num=1;$num<=$numTuhmbnails;$num++) {
            ...
            $fullThumbnailPath = "$pathToThumbnail/$videoId-$imageName" ; //full path to thumbnail
                ...
                $query = $this->con->prepare("INSERT INTO thumbnails(videoId,filePath,selected) VALUES(:videoId,:filePath,:selected)"); //insert thumbnail data into database

                $query->bindParam(":videoId",$videoId); 
                $query->bindParam(":filePath",$fullThumbnailPath);
                $query->bindParam(":selected",$selected);

                $selected = ($num == 1) ? 1 : 0; //select first thumbnail by default
    ...
    return (int)shell_exec("$this->ffprobePath -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 \"$filePath\""); //get video duration using ffmpeg
